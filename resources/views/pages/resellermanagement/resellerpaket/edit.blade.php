...
<div class="main-content">
    <section class="section">
        ...
        <div class="card">
            <form action="{{ route('resellerdata-paket.update',['id'=>$resellerpaket->id]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="form-group col-12 col-md-6">
                            ...
                            <input type="text" inputmode="numeric" name="price" inputmode="numeric" value="{{ $resellerpaket->price }}"
                        ...
                        <div class="form-group col-12 col-md-6">
                            ...
                            <select class="form-control select2 @error('reseller_id') is-invalid @enderror"
                                ...
                                <option value="{{ $s->id }}" {{ $s->id == $resellerpaket->reseller_id ? 'selected' : '' }}>{{ $s->name }}
                                </option>
                                @endforeach
                            </select>
                            @error('reseller_id')
                        ...
                        <div class="form-group col-12 col-md-6">
                            ...
                            <select class="form-control select2 @error('paket_id') is-invalid @enderror"
                                ...
                                @foreach ($paket as $s)
                                <option value="{{ $s->id }}" {{ $s->id == $resellerpaket->paket_id ? 'selected' : '' }}>{{ $s->name }} - {{ number_format($s->price) }}
                                </option>
                                @endforeach
                            </select>
                            @error('paket_id')
                        ...
                        <div class="form-group col-12 col-md-6">
                            ...
                            <select class="form-control select2" id="extension" name="status">
                                ...
                                <option value="1" {{ $resellerpaket->status == 1 ? 'selected' : '' }}>Aktif</option>
                                <option value="0" {{ $resellerpaket->status == 0 ? 'selected' : '' }}>Tidak Aktif</option>
                            </select>
                            @error('status')
                            <div class="invalid-feedback">
                                {{ $message }}
